...
public class PersistChanGrpMgr {
    ...
    private static final int DEFAULT_STARTDELAY = 1000; // ms
    ...
    private static final Log LOG = LogFactory.getLog(PersistChanGrpMgr.class);
    ...
    public ChannelGroup createGroup(String name) {
        LOG.debug("Creating Persistent Group: " + name);
        ...
        LOG.debug("createGroup(\"" + name + "\" yielded: " + result);
        return result;
    }
    ...
    public void deleteGroup() {
        ...
        LOG.debug("Deleting Persistent Group: " + group.getTitle());
        ...
        synchronized (builder) {
            try {
                ...
                Channel[] chans = (Channel[]) group.getChannels().toArray(new Channel[group.getChannels().size()]);
                ...
                LOG.error("Unable to delete Persistent Group: " + e.getMessage());
    ...
    public Channel addChannel(String url) {
        ...
        synchronized (builder) {
            try {
                ...
                if (achannel == null) { // Channel is not in the database
                    achannel = newChannel(url);
                    LOG.debug("Added New Channel: " + url);
                } else { // Channel is in the database, but it may not already be in this group
                    if (!hasChannel(achannel)) {
                        LOG.debug("Loaded existing channel" + url);
                        group.add(achannel);
    ...
    */
    ...
    public void notifyItems(Channel channelHandle) {
        if (globalChannelObserver != null) {
            for (ItemIF itemIF : channelHandle.getItems()) {
                notifyItemAdded((Item) itemIF);
            }
        }
    }
    ...
    public void notifyChannelsAndItems() {
        for (ChannelIF channelIF : group.getChannels()) {
            notifyChannelsAndItems((Channel) channelIF);
        }
    }
    ...
    public synchronized void deActivate(final boolean waitForFinish) {
        if (task != null)
            LOG.debug("deActivate(" + task.getName() + ") " + activated);
        else
            LOG.debug("deActivate task = null");
    ...
    private Channel findChannel(String url) {
        ...
        synchronized (builder) {
            ...
            try {
                ...
                if (size > 0) {
                    if (size > 1) {
                        LOG.error("Multiple Channels for " + url + " found.");
                    }
        ...
        LOG.info("findChannel: " + url + "->" + achan);
        return achan;
    }
    ...
    private ChannelGroup findChannelGroup(String name) {
        ...
        synchronized (builder) {
            try {
                ...
                if (size > 0) {
                    if (size > 1) {
                        LOG.error("Multiple Channel Groups called " + name + " found.");
                    }
        ...
        LOG.debug("findChannelGroup: " + name + "->" + result);
        return result;
    }
}
