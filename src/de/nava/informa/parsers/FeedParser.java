    ...
    private static synchronized ChannelIF parse(ChannelBuilderIF cBuilder, Document doc) throws ParseException {
        ...
        if (rootElement.startsWith("rss")) {
            ...
            if (rssVersion.indexOf("0.91") >= 0) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses RSS root element (Version 0.91).");
                }
                return RSS_0_91_Parser.getInstance().parse(cBuilder, root);
            } else if (rssVersion.indexOf("0.92") >= 0) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses RSS root element (Version 0.92).");
                }
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses RSS root element (Version 0.93).");
                }
                LOG.info("RSS 0.93 not fully supported yet, fall back to 0.91.");
                // TODO: support RSS 0.93 when aware of all subtle differences.
            } else if (rootElement.indexOf("0.94") >= 0) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses RSS root element (Version 0.94).");
                }
                LOG.info("RSS 0.94 not fully supported yet, will use RSS 2.0");
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses RSS root element (Version 2.0).");
                }
                return RSS_2_0_Parser.getInstance().parse(cBuilder, root);
            } else {
            ...
            if (versionAttr != null) {
                ...
                if (feedVersion.indexOf("0.1") >= 0 || feedVersion.indexOf("0.2") >= 0) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Channel uses feed root element (Version " + feedVersion + ").");
                    }
                    LOG.info("This atom version is not really supported yet, assume Atom 0.3 format");
                    return Atom_0_3_Parser.getInstance().parse(cBuilder, root);
                } else if (feedVersion.indexOf("0.3") >= 0) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Channel uses feed root element (Version 0.3).");
                    }
                    return Atom_0_3_Parser.getInstance().parse(cBuilder, root);
                }
            } else if (namespace != null && namespace.getURI() != null) {
                if (!"http://www.w3.org/2005/Atom".equals(namespace.getURI())) {
                    LOG.info("Channel uses unknown namespace in feed root element, assume Atom 1.0 format.");
                } else if (LOG.isDebugEnabled()) {
                    LOG.debug("Channel uses feed root element (Atom 1.0 format).");
                }
