...
class RSS_1_0_Parser implements ChannelParserIF {
    ...
    private static final Log LOG = LogFactory.getLog(RSS_1_0_Parser.class);
    ...
    public ChannelIF parse(ChannelBuilderIF cBuilder, Element root) throws ParseException {
        ...
        if (defNS == null) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("No default namespace found.");
            }
        }
        ...
        // 0..1 Rating element
        final Element rating = channel.getChild("rating", rss091NS);
        if (rating != null) {
        ...
        if (elGenerator != null) {
            final Attribute generator = elGenerator.getAttribute("resource", ParserUtils.getNamespace(elGenerator, "rdf"));
            if (generator != null) {
        ...
        final Element updatePeriod = channel.getChild("updatePeriod", syNS);
        ...
        final Element updateFrequency = channel.getChild("updateFrequency", syNS);
        if (updateFrequency != null) {
            chnl.setUpdateFrequency(new Integer(updateFrequency.getTextTrim()));
        }

        // 0..1 update base element
        final Element updateBase = channel.getChild("updateBase", syNS);
        if (updateBase != null) {
