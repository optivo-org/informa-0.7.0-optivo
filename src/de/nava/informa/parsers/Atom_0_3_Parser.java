...
class Atom_0_3_Parser implements ChannelParserIF {
    private final static Log LOG = LogFactory.getLog(Atom_0_3_Parser.class);
    ...
    public ChannelIF parse(ChannelBuilderIF cBuilder, Element channel)
        ...
        if (defNS == null) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.info("No default namespace found.");
            }
        }
        ...
        if (dcNS == null) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("No namespace for dublin core found");
            }
            dcNS = defNS;
        }
        ...
        if (channel.getAttribute("version") != null) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Atom version " + formatVersion + " specified in document.");
            }
        } else if (LOG.isDebugEnabled()) {
            LOG.debug("No format version specified, using default.");
        }
        ...
        */
        ...
        while (i.hasNext()) {
            ...
            // get title element
            // TODO : deal with type attribute
            Element elTitle = item.getChild("title", defNS);
            String strTitle = "<No Title>";

            if (elTitle != null) {
                strTitle = getTitle(elTitle);
                LOG.debug("Parsing title " + elTitle.getTextTrim() + "->" +
                        strTitle);
            }

            if (LOG.isDebugEnabled()) {
                LOG.debug("Entry element found (" + strTitle + ").");
            }
